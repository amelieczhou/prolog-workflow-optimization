	...
	for(int i=0; i<overflowdcs.size(); i++){
		...
		do{
			...
			for(int j=0; j<underflowdcs.size(); j++){//choose one cheapest
				...
				if(!underflowdcs[j]->evalConstraint()){
					float curcost = 0;
			...
			if(migto != -1){
				...
				//update tempworkload
				dcs[to]->tempworkload.push_back(dcs[from]->tempworkload.back());
...
/*	for(int i=0; i<numDCs; i++){
	...
	for(int dciter=0; dciter<numDCs; dciter++){
		...
		#pragma omp parallel
		{
			...
			for(int globaliter=0; globaliter<randomsize; globaliter++){
				...
				do{	
					...
					if (VMTP.size()> max_VM[dciter]) //count how many vms during execution
						max_VM[dciter] = VMTP.size();
		...
		operacost += ave_cost;
		printf("max # VM in datacenter %d is %d\n", dciter,max_VM[dciter]);
	}//dcs
