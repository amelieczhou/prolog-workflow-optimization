...
void Autoscaling::Initialize(){
	...
	for(; vp.first!=vp.second; ++vp.first) {//edge weight for communication cost		
		Vertex v1 = *vp.first;
		//(*dags[0]->g)[v1].prefer_type = types-1;//types-1;
		int rnd = rand()%4;
		(*dags[0]->g)[v1].assigned_type = rnd;
	}
...
void Autoscaling::Simulate(bool autoscaling){
	...
	if(autoscaling){
		...
		bool debug = false;
		if(debug){
			rFile = fopen("net_google.csv","r");
			if(rFile == NULL){
				printf("cannot open net_google.csv\n");
				exit(1);
			}
			...
			for(int i=0; i<1; i++){
				for(int j=0; j<randomsize; j++){
					...
					random_network_down[i*randomsize+j] = random_tmp[i*10000+j];
				}
			}
		}else{
	...
	#pragma omp parallel
	{
		...
		for(int monte=0; monte < randomsize; monte++)
		{
			...
			int max_VM[types]={0,0,0,0};
			float cost_VM[types] = {0,0,0,0}; //the monetary cost spent on each type of VMs
			//EDF scheduling
			double t = 0;
			...
			do{	
				...
				for(int i=0; i<types; i++)//////////
				{
					...
					for(int j=0; j<size1; j++)
					{
						if(VMTP[i][j]->tk == NULL)
						{
							...
							cost_VM[i] += (priceOnDemand[i] * ceil(runtime / 3600.0));
							max_VM[i] += ceil(runtime / 3600.0);
				...
				for(int i=0; i<types; i++)
				{
					...
					//if(VMTP[i].size()> max_VM[i]) //count how many vms during execution
					//	max_VM[i] = VMTP[i].size();
				}
			...
			for(int i=0; i<types; i++)
			{
				...
				printf("VM hours of type %d is %d\n",i,max_VM[i]);
				printf("cost spent on type %d VM is %f\n", i, cost_VM[i]);
			}
